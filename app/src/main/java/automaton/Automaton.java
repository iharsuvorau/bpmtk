...
import graphs.ElementaryCyclesSearch;
...
public class Automaton {
  ...
  public Automaton(
      ...
      int initialState,
      IntHashSet FinalStates,
      BiMap<IntArrayList, IntArrayList> caseTracesMapping,
    ...
    //		PrintWriter pw = new PrintWriter("/Users/daniel/Documents/workspace/Master thesis paper
    // tests/Road Traffic/tracesContained.txt");
    ...
    //				for(IntArrayList potentialPath :
    // this.states.get(finalState).potentialPathsAndTraceLabels().get(finalConfiguration).keySet())
    //
  ...
  public Automaton(
      Map<Integer, State> states,
      BiMap<Integer, String> eventLabels,
      BiMap<String, Integer> inverseLabelMapping,
      Map<Integer, Transition> transitions,
      int initialState,
      IntHashSet FinalStates,
  ...
  public Map<IntIntHashMap, List<IntArrayList>> configCasesMapping() {
    if (this.configCaseMapping == null) this.configCaseMapping = new UnifiedMap<>();
    return this.configCaseMapping;
  }

  private void discoverFinalConfigurationsForModel()
      throws FileNotFoundException // (boolean discoverPotentialPaths)
      {
    boolean[][] adjacencyMatrix =
        new boolean[Collections.max(this.states().keySet()) + 1]
            [Collections.max(this.states().keySet()) + 1];
    ...
    for (int key : this.states().keySet()) graphNodes[key] = this.states().get(key);
    @SuppressWarnings("unchecked")
    List<List<State>> cycles =
        (List<List<State>>)
            new ElementaryCyclesSearch(adjacencyMatrix, graphNodes).getElementaryCycles();
    cycles.removeIf(states1 -> states1.size() == 1);
    for (int i = 0; i < Collections.max(this.states().keySet()) + 1; i++) {
      if (adjacencyMatrix[i][i]) {
        List<State> selfLoop = new FastList<>();
    ...
    UnifiedSet<IntIntHashMap> elemLoopLabels = new UnifiedSet<>();
    for (int i = 0; i < cycles.size(); i++) {
      oldLoopLabels = new UnifiedSet<>();
      ...
      for (State state : cycle)
        for (IntIntHashMap loopLabels : newLoopLabels) ((State) state).loopLabels().add(loopLabels);
    }
    ...
    IntObjectHashMap<List<Transition>> compOutArcs = new IntObjectHashMap<>();
    ...
    //			{
    //				pw.printf("%d -> %d [EID=\"%s\"];%n", tr.source().component(), c,
    // this.eventLabels().get(tr.eventID()));
    //			}
    //		}
    //		pw.println("}");
    //		pw.close();
    ...
    if (this.finalConfigurations == null) this.finalConfigurations = new UnifiedSet<>();
    ...
    for (int finalState : this.finalStates().toArray()) {
      ...
      for (List<Transition> trs : compInArcs.values()) for (Transition tr : trs) tr.explore = true;
      for (Transition tr : compInArcs.get(fState.component())) {
        tr.explore = false;
        State trSource = tr.source();
        if (trSource.isLoopState) {
          Set<IntIntHashMap> uniqueCycles = new UnifiedSet<>();
    ...
    while (!toBeVisited.isEmpty()) {
      ...
      if (!state.loopLabels().isEmpty()) state.futureLoops().addAll(state.loopLabels());
    ...
    for (IntIntHashMap finalConfiguration : this.finalConfigurations) {
      ...
      for (int key : test.keySet().toArray()) if (test.get(key) >= 200) test.put(key, 1);
  ...
  private void calculateLogFutures() {
    ...
    //					}
    //				}
    //				f_state.potentialPathsAndTraceLabels().put(finalConfiguration, new
    // UnifiedMap<IntArrayList, IntArrayList>()); //potentialPaths.get(finalState)
    //				//System.out.println(finalState +" - " + finalConfiguration + " - " +
    // potentialPaths.get(finalState));
    ...
    //					f_state.potentialPathsAndTraceLabels().get(finalConfiguration).put(potentialPath,
    // relevantTraceLabels);
      ...
      if (state.isLoopState)
        if ((size = state.loopLabels().size()) >= 2) {
          do {
            ...
            keyList = new FastList<>(state.loopLabels());
              ...
              for (int loopIt2 = 0; loopIt2 < keyList.size(); loopIt2++) {
                ...
                if (state.loopLabels().add(combinedLoopLabels)) {
                  if ((loop1 = loopLabelsStatesMapping.get(keyList.get(loopIt1))) == null)
                    loop1 = new FastList<>();
                  Set<State> combinedStates = new UnifiedSet<>(loop1);
                  if ((loop2 = loopLabelsStatesMapping.get(keyList.get(loopIt2))) != null)
                    combinedStates.addAll(loop2);
                  for (State st : combinedStates) st.loopLabels().add(combinedLoopLabels);
                }
              }
  ...
  public void toDot(PrintWriter pw) throws IOException {
    ...
    for (State n : this.states.values()) {
      ...
      for (Transition t : n.outgoingTransitions()) {
        pw.printf(
            "%d -> %d [EID=\"%s\"];%n",
            n.id(), t.target().id(), this.eventLabels().get(t.eventID()));
      }
