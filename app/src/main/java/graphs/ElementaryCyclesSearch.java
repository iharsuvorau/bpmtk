...
/**
 * Searchs all elementary cycles in a given directed graph. The implementation is independent from
 * the concrete objects that represent the graphnodes, it just needs an array of the objects
 * representing the nodes the graph and an adjacency-matrix of type boolean, representing the edges
 * of the graph. It then calculates based on the adjacency-matrix the elementary cycles and returns
 * a list, which contains lists itself with the objects of the concrete graphnodes-implementation.
 * Each of these lists represents an elementary cycle.<br>
 * <br>
 *
 * <p>The implementation uses the algorithm of Donald B. Johnson for the search of the elementary
 * cycles. For a description of the algorithm see:<br>
 * Donald B. Johnson: Finding All the Elementary Circuits of a Directed Graph. SIAM Journal on
 * Computing. Volumne 4, Nr. 1 (1975), pp. 77-84.<br>
 * <br>
 *
 * <p>The algorithm of Johnson is based on the search for strong connected components in a graph.
 * For a description of this part see:<br>
 * Robert Tarjan: Depth-first search and linear graph algorithms. In: SIAM Journal on Computing.
 * Volume 1, Nr. 2 (1972), pp. 146-160.<br>
 *
