...
/**
 * This is a helpclass for the search of all elementary cycles in a graph with the algorithm of
 * Johnson. For this it searches for strong connected components, using the algorithm of Tarjan. The
 * constructor gets an adjacency-list of a graph. Based on this graph, it gets a nodenumber s, for
 * which it calculates the subgraph, containing all nodes {s, s + 1, ..., n}, where n is the highest
 * nodenumber in the original graph (e.g. it builds a subgraph with all nodes with higher or same
 * nodenumbers like the given node s). It returns the strong connected component of this subgraph
 * which contains the lowest nodenumber of all nodes in the subgraph.<br>
 * <br>
 *
 * <p>For a description of the algorithm for calculating the strong connected components see:<br>
...
public class StrongConnectedComponents {
  ...
  private Vector getLowestIdComponent() {
    ...
    for (Object currentSCC : this.currentSCCs) {
      Vector scc = (Vector) currentSCC;
