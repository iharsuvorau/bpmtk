package processmining.optimization;
...
import processmining.splitminer.dfgp.DFGEdge;
...
public class SimpleDirectlyFollowGraph extends DirectlyFollowGraphPlus {
  ...
  public SimpleDirectlyFollowGraph(BitSet matrixSDFG, SimpleLog slog, int size) {
    ...
    //        the cell (i,j) is set to TRUE if there exists an edge in the DFG with srcID = i and
    // tgtID = j
  ...
  public SimpleDirectlyFollowGraph(
      DirectlyFollowGraphPlus directlyFollowGraphPlus, boolean tabuSearch) {
  ...
  public Set<Integer> getTabuSet() {
    return this.tabu;
  }

  public void setTabuSet(Set<Integer> tabu) {
    this.tabu = new HashSet<>(tabu);
  }

  public void setParallelisms(Map<Integer, HashSet<Integer>> parallelisms) {
  ...
  public BPMNDiagram convertIntoBPMNDiagram() {
    ...
    node =
        diagram.addEvent(
            "0", Event.EventType.START, Event.EventTrigger.NONE, Event.EventUse.CATCH, true, null);
    mapping.put(0, node);

    for (int taskID = 1; taskID < (size - 1); taskID++) {
      node =
          diagram.addActivity(
              Integer.toString(taskID), loopsL1.contains(taskID), false, false, false, false);
      mapping.put(taskID, node);
    }

    node =
        diagram.addEvent(
            "-1", Event.EventType.END, Event.EventTrigger.NONE, Event.EventUse.THROW, true, null);
