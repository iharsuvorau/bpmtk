  ...
  public void computePercentages() {
    ...
    for (int i = 0; i < totalActivities; i++) {
      for (int j = 0; j < totalActivities; j++) {
        relativeDFG[i * totalActivities + j] =
            (double) dfg[i * totalActivities + j] / activityObserved[i];
        relativeConcurrencyMatrix[i * totalActivities + j] =
            (double) concurrencyMatrix[i * totalActivities + j]
                / (activityObserved[i] + activityObserved[j]);
      }
    }
  }

  public double[] getRelativeConcurrencyMatrix() {
    return relativeConcurrencyMatrix;
  }

  public double[] getRelativeDFG() {
    return relativeDFG;
  }
