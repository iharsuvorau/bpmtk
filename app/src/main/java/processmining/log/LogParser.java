...
public class LogParser {
  ...
  public static SimpleLog getSimpleLog(String path) {
    ...
    HashMap<String, Integer> labelsToIDs =
        new HashMap<>(); // this maps the original name of an event to its code
    HashMap<Integer, String> events =
        new HashMap<>(); // this maps the code of the event to its original name
    HashMap<String, Integer> reverseMap = new HashMap<>(); // this maps the event name to its code
    HashMap<String, Integer> traces =
        new HashMap<>(); // this is the simple log, each trace is a string associated to its
    // frequency
  ...
  public static SimpleLog getSimpleLog(XLog log, XEventClassifier xEventClassifier) {
    ...
    //        for( String t : traces.keySet() ) System.out.println("DEBUG - ["+ traces.get(t) +"]
    // trace: " + t);

    //        System.out.println("DEBUG - final mapping:");
    //        for( int code : events.keySet() ) System.out.println("DEBUG - " + code + " = " +
    // events.get(code));
  ...
  public static SimpleLog getComplexLog(XLog log, XEventClassifier xEventClassifier) {
    ...
    // when real concurrencies are available, the directly-follow relations slightly differ from the
    // simple case
    ...
    events.put(STARTCODE, "autogen-start");
    //        NOTE: for complex logs (with activities life-cycle), we overwrite the standard ENDCODE
    // later.
    //        events.put(ENDCODE, "autogen-end");
    ...
    for (tIndex = 0; tIndex < totalTraces; tIndex++) {
      ...
      for (eIndex = 0; eIndex < traceSize; eIndex++) {
        event = trace.get(eIndex);
        //                System.out.println("DEBUG " + count++ + "- lifecycle: " +
    ...
    if (Math.abs(startEvents - completeEvents) < ((double) totalEvents * 0.50)) {
      ...
      for (int i = 0; i < totalActivities; i++)
        for (int j = 0; j < i; j++) {
          if (exclusiveness[i * totalActivities + j] != 0
              && parallelism[i * totalActivities + j] != 0) {
            potentialORs[i * totalActivities + j]++;
            potentialORs[j * totalActivities + i]++;
            //                        System.out.println("DEBUG - potential OR (" +
            // exclusiveness[i*totalActivities + j] + "," + parallelism[i*totalActivities + j] +") "
  ...
  public SimpleLog getSimpleLog(XLog log, XEventClassifier xEventClassifier, double percentage) {
    ...
    for (String trace : sortedTraces.keySet()) {
      if (parsed < maxTraces) {
        //                System.out.println("DEBUG - trace, frequency: " + trace + "," +
        // traces.get(trace) );
