...
package processmining.splitminer.dfgp;
...
public class DirectlyFollowGraphPlus {
  ...
  public DirectlyFollowGraphPlus(SimpleLog log) {
    this(
        log,
        DFGPUIResult.FREQUENCY_THRESHOLD,
        DFGPUIResult.PARALLELISMS_THRESHOLD,
        DFGPUIResult.STD_FILTER,
        DFGPUIResult.PARALLELISMS_FIRST);
  }

  public DirectlyFollowGraphPlus(
      SimpleLog log,
  ...
  public Set<Integer> getLoopsL1() {
    return loopsL1;
  }

  public Map<Integer, HashSet<Integer>> getParallelisms() {
    return parallelisms;
  }
  ...
  public BPMNDiagram getDFGP(boolean labels) {
    ...
    for (int event : nodes.keySet()) {
      ...
      task =
          diagram.addActivity(
              (labels ? label : Integer.toString(event)), false, false, false, false, false);
    ...
    for (int event : nodes.keySet()) {
      ...
      if (event == startcode || event == endcode)
        node =
            diagram.addEvent(
                label,
                (event == startcode ? Event.EventType.START : Event.EventType.END),
                Event.EventTrigger.NONE,
                (event == startcode ? Event.EventUse.CATCH : Event.EventUse.THROW),
                true,
                null);
      else node = diagram.addActivity(label, loopsL1.contains(event), false, false, false, false);
  ...
  public boolean areConcurrent(int A, int B) {
    //        if( log instanceof ComplexLog ) return concurrencyMatrix[A*log.getEvents().size() +B]
  ...
  public void buildDFGP() {
    System.out.println(
        "DFGP - settings (eta, epsilon, filter-type) > "
  ...
  public void detectLoops() {
    ...
    for (DFGEdge e1 : edges) {
      ...
      //            if src OR tgt are length 1 loops, we do not evaluate length 2 loops for this
      // edge,
      //            because a length 1 loop in parallel with something else
      //            can generate pattern of the type [src :: tgt :: src] OR [tgt :: src :: tgt]
      if (!loopsL2.contains(e1)
          && dfgp.get(tgt).containsKey(src)
          && !loopsL1.contains(src)
          && !loopsL1.contains(tgt)) {
        ...
        for (String trace : traces.keySet()) {
          src2tgt_loop2Frequency +=
              (StringUtils.countMatches(trace, src2tgt_loop2Pattern) * traces.get(trace));
          tgt2src_loop2Frequency +=
              (StringUtils.countMatches(trace, tgt2src_loop2Pattern) * traces.get(trace));
        }
        ...
        //                [src :: tgt :: src] OR [tgt :: src :: tgt], so we set both edges as
        // short-loops
        if (loop2score != 0) {
          loopsL2.add(e1);
          loopsL2.add(e2);
  ...
  private void generateBitmatrixSplits() {
    ...
    //        for each split task, we have a matrix of bits, each column being a combination of
    // successors tasks that are executed
    HashMap<Integer, Matrix> splitMaps = new HashMap<>();
    //        for each split task, we have an array storing the successors ids
    //        we query the array to know the index of the successor to update accordingly the row of
    // a matrix
    HashMap<Integer, ArrayList<Integer>> successors = new HashMap<>();

    //        these two structures keep track of the mapping between integer successors codes and
    // the BPMN nodes
    //        as well as the incoming edge of each successor, this is fundamental to edit the BPMN
    // diagram later
    ...
    //        while parsing each trace, we have to keep track of all the split tasks that we
    // encountered
    ...
    //        we can set a max distance after which we do not update anymore the bitarray for that
    // split task
    ...
    for (String t : traces.keySet()) {
      ...
      while (trace.hasMoreTokens()) {
        ...
        for (int sti : splitTasksInTrace.keySet()) {
          ...
          //                    however, split task executed too long ago or same of the current
          // event are not taken into account
          //                    distances.put(sti, distances.get(sti)+1);
          if (distances.get(sti) > MAXD || event == sti) {
            skipcounter++;
            continue;
          }
          //                    if the current event is a successor for one or more of them (in
          // which case the event is also a join)
      ...
      //            once the whole trace has been parsed, we add the bitset of each split task to
      // its matrix of bitsets
      for (int sti : splitTasksInTrace.keySet())
        splitMaps.get(sti).addBitset(splitTasksInTrace.get(sti), traceFrequency);
    }
  ...
  private void generateSplitsHierarchyFromObservationMatrix(int split, Matrix matrix) {
    ...
    for (Pair<Integer, Integer> p : ANDs) {
      //                if(removableSuccessors.contains(p.getLeft()) ||
      // removableSuccessors.contains(p.getRight())) continue;
      ...
      if (relations.containsKey(p) && relations.get(p) != Gate.AND)
        System.out.println(
            "WARNING - double relation for: (" + p.getLeft() + "," + p.getRight() + ")");
    ...
    for (Pair<Integer, Integer> p : XORs) {
      ...
      //                transposedMatrix.put(p.getLeft()*100, bs0);
      if (relations.containsKey(p) && relations.get(p) != Gate.XOR)
        System.out.println(
            "WARNING - double relation for: (" + p.getLeft() + "," + p.getRight() + ")");
    ...
    for (Pair<Integer, Integer> p : ORs) {
      ...
      for (int i = 0; i < ROWS; i++) bs0.set(i, bs1.get(i) || bs2.get(i));
      //                transposedMatrix.put(p.getLeft()*100, bs0);
      if (relations.containsKey(p) && relations.get(p) != Gate.OR)
        System.out.println(
            "WARNING - double relation for: (" + p.getLeft() + "," + p.getRight() + ")");
  ...
  private Gate determineRelation(
      int s1, int s2, BitSet bs1, BitSet bs2, int size, Set<Integer> skips) {
    Gate type = Gate.OR;
    boolean safe = true;
    int mismatch = 0;
    int match = 0;
    ...
    for (int i = 0; i < size; i++) observations.add(new ImmutablePair<>(bs1.get(i), bs2.get(i)));
  ...
  public void detectParallelismsOnDFG() {
    ...
    for (DFGEdge e1 : edges) {
      ...
      if (parallelismsFirst) priorityCheck = !loopsL2.contains(e1);
        ...
        priorityCheck = !loopsL2.contains(e1) && !loopsL1.contains(src) && !loopsL1.contains(tgt);
      ...
      if (dfgp.get(tgt).containsKey(src) && priorityCheck && !removableEdges.contains(e1)) {
        ...
        parallelismScore =
            (double) (src2tgt_frequency - tgt2src_frequency)
    ...
    //        System.out.println("DFGP - parallelisms found (total, confirmed): (" +
    // totalParallelisms + " , " + confirmedParallelisms + ")");
  }
  ...
  public void removeEventSubprocesses() {
    ...
    for (int n : nodes.keySet())
      if (parallelisms.containsKey(n) && parallelisms.get(n).size() >= maxP)
        eventSubprocesses.add(n);
  ...
  private void addNode(DFGNode n) {
    ...
    nodes.put(code, n);
    if (!incomings.containsKey(code)) incomings.put(code, new HashSet<>());
    if (!outgoings.containsKey(code)) outgoings.put(code, new HashSet<>());
    if (!dfgp.containsKey(code)) dfgp.put(code, new HashMap<>());
  }
  ...
  public void printEdges(boolean includeL1) {
    ...
    for (DFGEdge e : edges) {
      edge =
          events.get(e.getSourceCode())
              + " > "
              + events.get(e.getTargetCode())
              + " ["
              + e.getFrequency()
              + "]";
  ...
  public void printNodes() {
    for (DFGNode n : nodes.values()) System.out.println("DEBUG - node : " + n.print());
  }
...
class Matrix {
  ...
  void addBitset(BitSet combo, int frequency) {
    ...
    matrix.merge(combo, frequency, Integer::sum);
  }
