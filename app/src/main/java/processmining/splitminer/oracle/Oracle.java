...
package processmining.splitminer.oracle;
...
public class Oracle {
  ...
  public OracleItem getFinalOracleItem(Set<OracleItem> oracleItems) {
    ...
    while (oracleItems.size() != 1) {
      ...
      //            for( OracleItem oi : oracleItems ) System.out.println("DEBUG - oracle item: " +
      // oi);
      merged = false;
      ...
      while (true) {
        ...
        for (OracleItem oi : oracleItems) {
          //                    if we found oracle items to be merged we add the Oracle item we were
          // analysing
          //                    to the toMerge set and we proceed with the merging
          for (OracleItem oii : oracleItems) if ((oi != oii) && oi.isXOR(oii)) toMerge.add(oii);

          //                    if we found some Oracle item that can be merged we add also the one
      ...
      if (!merged) {
        ...
        //                System.out.println("WARNING - impossible merging oracle items, extending
        // the concurrency relationships");

        //                if we have to force a merging, we try to merge the minimum distance couple
        // of Oracle items
        //                that is: we are trying to introduce the less possible changes in the
