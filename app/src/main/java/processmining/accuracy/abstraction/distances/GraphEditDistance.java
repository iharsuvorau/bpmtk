package processmining.accuracy.abstraction.distances;
...
public class GraphEditDistance {
  ...
  public double averageDistance(
      Collection<Subtrace> subtraces1, Collection<Subtrace> subtraces2, int order) {
  ...
  public double getUnbalancedSubtracesDistance(
      Collection<Subtrace> subtraces1, Collection<Subtrace> subtraces2, double order) {
    double[][] matrix;
    double[][] umatrix;
    double distance = 0;
  ...
  public double getSubtracesDistance(
      Collection<Subtrace> subtraces1, Collection<Subtrace> subtraces2, double order) {
    double[][] matrix;
    double distance = 0;
    int rows = subtraces1.size();
  ...
  public double getFreqWeightedSubtracesDistance(
      Collection<Subtrace> subtraces1,
      Collection<Subtrace> subtraces2,
      double order,
      double globalGramsCount) {
    double[][] matrix;
    double distance = 0;
    int rows = subtraces1.size();
    int cols = subtraces2.size();
    ...
    for (Subtrace st1 : subtraces1) {
      ...
      for (Subtrace st2 : subtraces2) {
        st2ia = st2.printIA();
        matrix[r][c] =
            ((double) Levenshtein.arrayDistance(st1ia, st2ia) / order) * st1.getFrequency();
        c++;
      }
      r++;
    }
  ...
  public double getDistance(Set<Edge> edges1, Set<Edge> edges2) {
    ...
    for (Edge e1 : edges1) {
      ...
      for (Edge e2 : edges2) {
        ...
        tgt2 = e2.getTGT();
        ds =
            (double) Levenshtein.stringDistance(src1, src2) / (double) Math.max(ls1, src2.length());
        dt =
            (double) Levenshtein.stringDistance(tgt1, tgt2) / (double) Math.max(lt1, tgt2.length());
        d = (dt + ds) / 2.0;
        matrix[r][c] = d;
        c++;
      }
