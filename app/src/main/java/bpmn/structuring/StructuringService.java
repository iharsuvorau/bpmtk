...
package bpmn.structuring;

import bpmn.helper.DiagramHandler;
import bpmn.structuring.core.StructuringCore;
import java.util.*;
...
public class StructuringService {
  ...
  public BPMNDiagram structureDiagram(
      BPMNDiagram diagram,
      String policy,
      int maxDepth,
      int maxSolutions,
    ...
    try {
      ...
      System.out.println(
          "ERROR - impossible structure diagram, returning the input: " + e.getMessage());
      return diagram;
    }
  ...
  private void structureDiagram() throws Exception {
    ...
    /**
     * STEP1: restructure the diagram in order to remove - double edges - fakeGates (those gate with
     * only 1 entering and 1 exiting flow) - multiple start and end events - boundary events
     */
    ...
    //        System.out.println("Total gateways after structuring: " + xorGates + "(xor) + " +
    // andGates + "(and) + " + orGates + "(or)." );
    ...
    for (Gateway g : new HashSet<>(diagram.getGateways()))
      diagramHandler.checkFakeGateway(diagram, g);
  ...
  private void removeMultipleStartEvents() {
    ...
    for (SubProcess sp : diagram.getSubProcesses()) sp2se.put(sp, new HashSet<>());
    for (Swimlane pl : diagram.getPools()) pl2se.put(pl, new HashSet<>());
    pl2se.put(null, new HashSet<>());
    ...
    for (SubProcess spe : sp2se.keySet())
      if (sp2se.get(spe).size() > 1) {
        ...
        System.out.println(
            "DEBUG - added fakeStart ("
                + fakeStart.getId()
                + ") for subProcess: "
                + (spe == null ? "null" : spe.getId()));
      }
  ...
  private void removeMultipleEndEvents() {
    ...
    for (SubProcess sp : diagram.getSubProcesses()) sp2ee.put(sp, new HashSet<>());
    for (Swimlane pl : diagram.getPools()) pl2ee.put(pl, new HashSet<>());
    pl2ee.put(null, new HashSet<>());
        ...
        if (e.getEventTrigger() == Event.EventTrigger.COMPENSATION) {
          for (Association association : diagram.getAssociations())
            if (association.getSource() == e)
              if (((association.getTarget() instanceof Activity)
                      && ((Activity) association.getTarget()).isBCompensation())
                  || ((association.getTarget() instanceof CallActivity)
                      && ((CallActivity) association.getTarget()).isBCompensation())) {
                System.out.println(
                    "DEBUG - Detaching compensation event and activity: "
                        + e.getId()
                        + " > "
                        + association.getTarget().getId());
                boundToFix.put(e, a);
          ...
          for (Flow f : diagram.getFlows())
            if (f.getSource().equals(a)) {
              ...
              if ((tgt instanceof Gateway)
                  && ((((Gateway) tgt).getGatewayType() == Gateway.GatewayType.EVENTBASED)
                      || (((Gateway) tgt).getGatewayType() == Gateway.GatewayType.DATABASED))) {
                g = (Gateway) tgt;
              } else {
                g =
                    diagram.addGateway(
                        "boundaryXOR", Gateway.GatewayType.DATABASED, a.getParentSubProcess());
  ...
  private void structureSubProcess(String processID, Collection<Flow> edges) {
    ...
    for (Flow flow : edges) {
      ...
      if (src instanceof DataObject
          || src instanceof Swimlane
          || src instanceof TextAnnotation
          || tgt instanceof DataObject
          || tgt instanceof Swimlane
          || tgt instanceof TextAnnotation) {
      ...
      if ((!tryClassicBPStruct)
          && (src instanceof Gateway)
          && (((Gateway) src).getGatewayType() == Gateway.GatewayType.PARALLEL)) {
        System.out.println("DEBUG - BPStruct enabled.");
        tryClassicBPStruct = true;
      }

      if (!originalNodes.containsKey(src.getId().toString()))
        originalNodes.put(src.getId().toString(), src);
      if (!originalNodes.containsKey(tgt.getId().toString()))
        originalNodes.put(tgt.getId().toString(), tgt);
    }

    try {
      iBPStruct spi =
          new iBPStruct(
              StructuringCore.Policy.valueOf(policy),
              maxDepth,
              maxSolutions,
              maxChildren,
              maxStates,
      ...
      System.out.println(
          "ERROR - impossible structure the current subprocess, setting diagram NULL.");
      e.printStackTrace();
    }
  ...
  private boolean rebuildSubProcess(String processID) {
    ...
    // System.out.println("Rebuilding: Process_" + processID + " with nodes(" + nodes.size() + ")
    // flows(" + flows.size() + ")");
    ...
    try {
      if (originalNodes.get(processID) instanceof SubProcess)
        parentProcess = (SubProcess) originalNodes.get(processID);
      ...
      for (BPMNNode n : nodes) {
        ...
        if (n instanceof Gateway) {
          ...
          if (greyList.contains(id)) {
            ...
            if (node == null)
              throw new Exception(
                  "Error parsing tasks: " + n.getLabel() + " cannot be duplicated!");
          } else {
          ...
          node = null;
          System.out.println(
              "ERROR - found new node that is not a gateway > "
      ...
      System.out.println(
          "Error rebuilding subProcess: "
              + processID
              + " - "
              + originalNodes.get(processID).getLabel());
      error = true;
    }
  ...
  private BPMNNode duplicateNode(BPMNNode node, SubProcess parentProcess, boolean blackMark) {
    ...
    if (blackMark) {
      if (!blackList.containsKey(id)) blackList.put(id, new HashSet<>());
    } else {
      // System.out.println("Duplicating (" + blackMark + "): " + node.getClass().getSimpleName() +
      // " : " + node.getId() + " with Parent: " + (parentProcess == null ? "top-Level" :
      // parentProcess.getId()));
      if (!whiteList.containsKey(id)) whiteList.put(id, new HashSet<>());
    }
    ...
    if (node instanceof SubProcess) {
      ...
      // System.out.println("("+ diagram.getFlows((SubProcess) node).size() +")STARTING>>> flows
      // duplication for: " + duplicate.getId() + " : " + node.getId());
      ...
      duplicate =
          diagram.addGateway(node.getLabel(), ((Gateway) node).getGatewayType(), parentProcess);
    ...
    // System.out.println("Added new node (" + duplicate.getClass().getSimpleName() + "): " +
    // duplicate.getId());
    return duplicate;
  }
  ...
  private void restoreEdges() {
    ...
    for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> e : unmappableEdges) {
      ...
      if (blackList.containsKey(srcID)
          || blackList.containsKey(tgtID)
          || whiteList.containsKey(srcID)
          || whiteList.containsKey(tgtID)) continue;
      ...
      if (e instanceof MessageFlow) diagram.addMessageFlow(src, tgt, (Swimlane) null, e.getLabel());
        ...
        diagram.addAssociation(src, tgt, ((Association) e).getDirection());
      else if (e instanceof DataAssociation) diagram.addDataAssociation(src, tgt, e.getLabel());

      // System.out.println(e.getClass().getSimpleName() + " restored: " + src.getId() + " > " +
      // tgt.getId());
    }

    for (BPMNNode n : unmappableNodes) {
      // System.out.println("Removing unmappable node: " + n.getClass().getSimpleName() + " : " +
      // n.getId());
      removeNode(n);
    }
  }
  ...
  private void restoreEvents() {
    ...
    for (Event e : boundToFix.keySet()) {
      ...
      if (compensationActivities.containsKey(e)) {
        ...
        if (blackList.containsKey(aID) || blackList.containsKey(eID)) {
          ...
          System.out.println(
              "["
                  + blackList.containsKey(aID)
                  + ":"
                  + blackList.containsKey(eID)
                  + "] unfixable bound between: "
                  + aID
                  + " > "
                  + eID);
    ...
    for (Event fse : fakeStartEvents) {
      ...
      for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> f :
          diagram.getFlows()) // loop on flows to find the fakeGateway of this fakeStartEvent
      if ((f.getSource() == fse) && (f.getTarget() instanceof Gateway)) {
          startFlow = f;
          g = (Gateway) f.getTarget();
          for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> ff :
              diagram.getFlows()) // loop on flows to find all the fakeStartFlows
          if (ff.getSource() == g) {
      ...
      if (g != null && !error) {
        ...
        for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> fff : fakeFlows)
          diagram.removeEdge(fff);
    ...
    for (Event fee : fakeEndEvents) {
      ...
      for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> f :
          diagram.getFlows()) // loop on flows to find the fakeGateway of this fakeEndEvent
      if ((f.getTarget() == fee) && (f.getSource() instanceof Gateway)) {
          endFlow = f;
          g = (Gateway) f.getSource();
          for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> ff :
              diagram.getFlows()) // loop on flows to find all the fakeEndFlows
          if (ff.getTarget() == g) {
