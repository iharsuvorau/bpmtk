...
package bpmn.structuring.graph;
...
public class Graph {
  ...
  static final boolean jsLimitation = true;
  ...
  public Graph(
      ...
      Map<String, Gateway.GatewayType> gateways) {
    this.gateways = gateways;
    ...
    incoming.put(
        this.entry,
        new ArrayList<>()); // this set will remain always empty, no incoming path to the main entry
    outgoing.put(
        this.exit,
        new ArrayList<>()); // this set will remain always empty, no outgoing path from the main
    // exit
  ...
  public Graph(Graph mould) {
    ...
    allPaths = new HashMap<>();
    for (int pid : mould.allPaths.keySet())
      allPaths.put(pid, new Path(pid, mould.allPaths.get(pid)));
  ...
  public void detectLoops() {
    ...
    exploreLoops(
        this.entry, unvisited, visiting, visitedGates, visitedEdges, loopEdges, forwardEdges);
  ...
  private boolean exploreLoops(
      ...
      HashMap<String, Boolean> visitedGates,
      HashSet<Integer> visitedEdges,
      HashSet<Integer> loopEdges,
      HashSet<Integer> forwardEdges) {
    String next;
    ...
    for (int pid : incoming.get(entry))
      if (!visitedEdges.contains(pid)) {
        visited = false;
        break;
      }
    ...
    for (String entry : brothers.keySet())
      for (String exit : brothers.get(entry).keySet())
        if ((size = (brotherhood = brothers.get(entry).get(exit)).size()) > 1) {
          // System.out.println("DEBUG - found " + brotherhood.size() + " brothers (entry: " + entry
          // + ") (exit: " + exit + ")");
          ntd = false;
          ...
          while (brotherhood.size() != 1) {
            ...
            // System.out.println("DEBUG - merged brothers [" + bigBrother.getPID() + " + " +
            // brotherhood.get(0) + "] with loop [" + bigBrother.isLoop() + ":" +
            // allPaths.get(brotherhood.get(0)).isLoop() + "]");
    ...
    for (String entry : brothers.keySet())
      for (String exit : brothers.get(entry).keySet()) {
        ...
        if (brothers.containsKey(exit) && brothers.get(exit).containsKey(entry)) {
          twinBrotherhood =
              brothers.get(exit).get(entry); // this can only be empty or with 1 element.
          if ((brotherhood.size() == 1) && (twinBrotherhood.size() == 1)) {
            // System.out.println("DEBUG - found reverse brothers (entry: " + entry + ") (exit: " +
            // exit + ")");
            ...
            if (bigBrother.isLoop() && twinBrother.isLoop()) {
              //                            System.out.println("WARNING - got double loop, merging
              // is dangerous.");
  ...
  private void concatenate(int firstPID, int secondPID) {
    ...
    // System.out.println("DEBUG - contatenating: " + firstPID + "("+ first.canConcat()+")" + " -> "
    // + secondPID);
  ...
  public Graph minimalDecomposition() {
    ...
    return detachGraph(
        allPaths.get(outgoing.get(entry).get(0)).getExit(),
        allPaths.get(incoming.get(exit).get(0)).getEntry(),
        gates);
  }
  ...
  public Set<Graph> decompose() {
    ...
    while (toVisit.size() != 0) {
      ...
      for (RPSTNode n : new HashSet<RPSTNode>(rpst.getChildren(root))) {
        switch (n.getType()) {
          case R:
            // System.out.println("DEBUG - found RIGID element (children: " +
            // rpst.getChildren(root).size() + " )");
            // toVisit.add(n);

            gates = new HashSet<>();
            for (IVertex v : new HashSet<IVertex>(n.getFragment().getVertices()))
              gates.add(v.getName());
  ...
  private Graph detachGraph(String entry, String exit, Set<String> gates) {
    ...
    // System.out.println("DEBUG - graph detached [incoming(entry): " + incoming.get(entry).size() +
    // "][outgoing(entry): " + outgoing.get(entry).size() + "]");
    // System.out.println("DEBUG - graph detached [incoming(exit): " + incoming.get(exit).size() +
    // "][outgoing(exit): " + outgoing.get(exit).size() + "]");
  ...
  public boolean recompose(Set<Graph> graphs) {
    ...
    for (Graph g : graphs) {
      ...
      if (g.getAlivePaths().size() != 1)
        System.out.println(
            "WARNING - maximum structured graph with: " + g.getAlivePaths().size() + " paths.");
  ...
  public boolean applyMove(Move move, String middleGate) {
        ...
        switch (move.getType()) {
          case PUSHDOWN:
            ...
            if ((gateways.get(entryGate) == Gateway.GatewayType.PARALLEL)
                && (gateways.get(exitGate) == Gateway.GatewayType.PARALLEL)) {
    ...
    try {
      if ((gateways.get(middleGate) == Gateway.GatewayType.PARALLEL)
          && (move.getType() == Move.MoveType.PULLUP)) {}
  ...
  public void updatePath(
      int pid, String newEntry, String oldEntry, String newExit, String oldExit) {
    ...
    if (!brothers.get(newEntry).containsKey(newExit))
      brothers.get(newEntry).put(newExit, new ArrayList<>());
