...
public class Path {
  ...
  public Set<Integer> getBrothers() {
    return brothers;
  }

  public Set<Integer> getReverseBrothers() {
    return reverseBrothers;
  }
  ...
  public boolean addBrother(Path brother) {
    ...
    if (brother.entry.equals(this.entry) && brother.exit.equals(this.exit)) {
      ...
      System.out.println(
          "ERROR - attempt to merge two NO brothers: " + this.PID + " - " + brother.PID);
      return false;
    }
  }
  ...
  public boolean addReverseBrother(Path reverseBrother) {
    ...
    if (reverseBrother.entry.equals(this.exit) && reverseBrother.exit.equals(this.entry)) {
      ...
      hasBrothers = true;
      // System.out.println("DEBUG - " + PID + ": got a new reverse brother (" +
      // reverseBrother.getPID() + ")");
      return true;
    } else {
      System.out.println(
          "ERROR - attempt to merge two NO reverse brothers: "
              + this.PID
              + " - "
              + reverseBrother.PID);
      return false;
  ...
  public boolean canConcat() {
    return !hasBrothers;
  }

  public boolean hasLoop() {
    return (!reverseBrothers.isEmpty() || loop);
  }
  ...
  public boolean concat(Path next) {
    ...
    if (this.exit.equals(next.entry) && !hasBrothers) {
      ...
      System.out.println(
          "ERROR - attempt to concatenate two NO consecutive paths: "
              + this.PID
              + " - "
              + next.PID);
      return false;
