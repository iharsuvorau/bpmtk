...
public class Structurer {
  ...
  public Graph getStructuredRigid(StructuringState iState) {
    ...
    if (solutions.isEmpty()) {
      ...
      System.out.println(
          "WARNING - no more move allowed. Maximum structuring reached with "
              + best.getGraph().getAlivePaths().size()
              + " paths.");
    ...
    switch (policy) {
      ...
      case BREADTH:
        ...
        while (!toVisit.isEmpty() && (depth <= maxDepth) && (solutions.size() <= maxSol)) {
          ...
          // System.out.println("DEBUG - [structurer: BREADTH] state(" + depth + ")(" +
          // solutions.size() + ")(" + zombies.size() + ")");
        }
        return;
      ...
      case ASTAR:
        ...
        while (true) {
          if (toVisitSorted.isEmpty()) {
            ...
            //                            for(StructuringState ss : zombies)
            // ss.getGraph().enablePullUp();
          ...
          if (timeBounded)
            if ((System.currentTimeMillis() - startingTime) > timeBound) {
              System.out.println(
                  "DEBUG - [structurer: A*] found "
                      + solutions.size()
                      + " solutions. Switching policy to LIMITED A*");
              break;
            }
        }
      ...
      case LIM_ASTAR:
        ...
        while (true) {
          ...
          // System.out.println("DEBUG - [structurer: LIMITED A*] state(" + toVisitQuick.size() +
          // ")(" + toVisitQuick.minCost() + ")" + "(" +  toVisitQuick.maxCost() + ")" + "(" +
          // solutions.size() + ")");
          if ((System.currentTimeMillis() - startingTime) > timeBound) {
            System.out.println(
                "DEBUG - [structurer: LIMITED A*] found "
                    + solutions.size()
                    + " solutions. Switching policy to LIMITED DEPTH");
            break;
          }
        }
        ...
        while ((toVisit.size() != 0)) {
          ...
          while (!tmpChildren.isEmpty()) {
            ...
            if (tmpChildren.remove(0).getCost() == 0) {
              tmpChildren = new ArrayList<>();
              //                            System.out.println("DEBUG - [structurer: LIMITED DEPTH]
              // move had cost 0.");
              break;
            }
          }
          ...
          if ((System.currentTimeMillis() - startingTime) > timeBound) {
            System.out.println(
                "DEBUG - [structurer: LIMITED DEPTH] found no solutions. Returning best state reached.");
