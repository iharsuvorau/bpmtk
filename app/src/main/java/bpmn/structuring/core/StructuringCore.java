...
public class StructuringCore {
  ...
  public Set<Graph> structureAll(Set<Graph> rigids) {
    structurer =
        new Structurer(policy, maxDepth, maxSol, maxChildren, maxStates, maxMinutes, timeBounded);
    structuredRigids = new HashSet<>();
    for (Graph g : rigids)
      structuredRigids.add(structurer.getStructuredRigid(new StructuringState(g, 0)));
