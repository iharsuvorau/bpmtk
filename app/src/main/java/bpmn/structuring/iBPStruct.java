...
import bpmn.structuring.graph.Graph;
import bpmn.structuring.graph.Path;
import java.util.*;
...
public class iBPStruct {
  ...
  public iBPStruct(
    ...
    if (policy == StructuringCore.Policy.ASTAR) {
      ...
      if (timeBounded) System.out.println("iBPStruct - [A*] max minutes: " + maxMinutes);
    }
  ...
  public boolean setProcess(Collection<BPMNNode> nodes, Collection<Flow> flows) {
    ...
    if ((ends.size() == 1) && (starts.size() == 1)) {
      ...
      for (String s : ends)
        System.out.println("DEBUG - extra end(" + s + "): " + this.nodes.get(s).getLabel());
      isValid = false;
    }
  ...
  public boolean structure() {
    ...
    core =
        new StructuringCore(
            policy, maxDepth, maxSol, maxChildren, maxStates, maxMinutes, timeBounded);
    ...
    while (toVisit.size() != 0) {
      ...
      for (String child : children.get(entry)) {
        ...
        if (!gateways.containsKey(tmpChild) && !tmpChild.equals(exit)) {
          // found a node with multiple children that is not a gateway OR a node with zero children
          // that is not the mainExit
  ...
  private BPMNNode unfoldPath(int pid, BPMNNode oEntry, BPMNNode oExit) {
    ...
    if ((entry instanceof Gateway) && (exit instanceof Gateway)) {
      ...
      // (!path.getReverseBrothers().isEmpty()));
      correctType = bestType(entryType, exitType, path.isLoop());
    }
    ...
    // System.out.println("DEBUG - unfolding path (" + path.getChain().size() + ")(" +
    // path.getBrothers().size() + ")(" + path.getReverseBrothers().size() + "): " + path.getPID() +
    // " - (" + oEntry + ":" + oExit + ")");
    ...
    if (path.getReverseBrothers().size() != 0) {
      ...
      // if( !(oEntry instanceof Gateway) || (((Gateway) oEntry).getGatewayType() !=
      ...
      // if( !(oExit instanceof Gateway) || (((Gateway) oExit).getGatewayType() !=
      // Gateway.GatewayType.DATABASED)  ) {
    ...
    if (exit != null && (!entry.equals(exit))) {
      ...
      return entry; // the exit of the last path in the chain if this path does not have brothers.
  }
  ...
  private Gateway.GatewayType bestType(
      Gateway.GatewayType entryType, Gateway.GatewayType exitType, boolean isLoop) {
    if (isLoop) return Gateway.GatewayType.DATABASED;
    // if( keepBisimulation ) return entryType;
    return entryType;
    // if( (entryType == Gateway.GatewayType.PARALLEL) || (exitType == Gateway.GatewayType.PARALLEL)
