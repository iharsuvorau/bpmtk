...
public class DiagramHandler {
  ...
  public void removeJoinSplit(BPMNDiagram diagram) {
    //        this method removes join/split gateways, transforming them into a sequence of a join
    // and a split
    Gateway split;
    ...
    do {
      ...
      if (keepGoing) {
        ...
        if (src instanceof Gateway
            && ((Gateway) src).getGatewayType() == Gateway.GatewayType.DATABASED) {
          newXOR = (Gateway) src;
        } else {
    ...
    do {
      ...
      for (Flow f : diagram.getFlows()) {
        ...
        if (src instanceof Gateway
            ...
            &&
            //                    ((Gateway) src).getGatewayType() == ((Gateway)
            // tgt).getGatewayType() &&
  ...
  public boolean checkDuplicates(BPMNDiagram diagram) {
    ...
    try {
      ...
      while (toAnalize.size() != 0) {
        ...
        } else if ((root.getType() == TCType.P)
            && (rootParent != null)
            && (rootParent.getType() == TCType.B)) {
          parentExit = gates.get(root.getExit().getName());
          if (!checkMap.containsKey(parentExit)) {
            checkMap.put(parentExit, new HashMap<>());
            hierarchy.put(parentExit, new ArrayList<>());
  ...
  private boolean mergeTrivials(
    ...
    for (BPMNNode survivor : toKeep) {
      ...
      if (!duplicates.get(label).isEmpty()) {
        Gateway gate =
            diagram.addGateway("", exit.getGatewayType(), survivor.getParentSubProcess());
  ...
  private boolean mergeBonds(
      Set<RPSTNode> bondsToCompare,
      HashMap<RPSTNode, String> rpstNodeToID,
      Map<String, BPMNNode> mapping,
      BPMNDiagram diagram) {
    ...
    for (RPSTNode bond : toKeep) {
      ...
      if (!duplicates.get(code).isEmpty()) {
        for (RPSTNode duplicateBond : duplicates.get(code))
          removeBond(duplicateBond, mapping, diagram, entry);
        changed = true;
  ...
  private void removeBond(
      RPSTNode bond, Map<String, BPMNNode> mapping, BPMNDiagram diagram, BPMNNode newEntry) {
  ...
  public void checkFakeGateway(BPMNDiagram diagram, Gateway g) {
    ...
    if (diagram.getInEdges(g).size() == 0 && diagram.getOutEdges(g).size() == 0)
  ...
  public void removeDoubleEdges(BPMNDiagram diagram) {
    ...
    for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> ff : towAway) {
      // System.out.println("DEBUG - doubleFlow removed: " + ff.getSource().getId() + " > " +
      // ff.getTarget().getId());
      diagram.removeEdge(ff);
    ...
    do {
      ...
      while (!eaten.contains(eater)) {
        ...
        for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> e : diagram.getOutEdges(eater))
          if ((e.getTarget() instanceof Gateway)
              && ((meal = (Gateway) e.getTarget()).getGatewayType() == eater.getGatewayType())) {
            unhealthy = false;
    ...
    do {
      ...
      while (!eaten.contains(eater)) {
        ...
        for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> e : diagram.getInEdges(eater))
          if ((e.getSource() instanceof Gateway)
              && ((meal = (Gateway) e.getSource()).getGatewayType() == eater.getGatewayType())) {
            unhealthy = false;
    ...
    for (BPMNNode n : originalToCopy.keySet()) {
      ...
      if ((n instanceof Event) && (copy instanceof Event)) {
        ...
        if (exceptionFor != null) {
          ...
          if (newExceptionFor instanceof Activity)
            ((Event) copy).setExceptionFor((Activity) newExceptionFor);
        }
      }
    }
  ...
  public BPMNNode copyNode(BPMNDiagram diagram, BPMNNode node, String label) {
    ...
    if (node instanceof SubProcess) {
      ...
      duplicate = diagram.addGateway(label, ((Gateway) node).getGatewayType(), (SubProcess) null);
  ...
  public boolean checkAndRemoveSkippingActivity(Activity a) {
    ...
    if ((prev instanceof Gateway)
        && (succ instanceof Gateway)
        && (successors.get(prev).size() == 2)) {
  ...
  public void setSkipping(Activity a) {
    ...
    for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> e : diagram.getInEdges(a))
      toRemove.add(e);
    for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> e : diagram.getOutEdges(a))
      toRemove.add(e);
