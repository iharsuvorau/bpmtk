...
public class GatewayMap {
  ...
  private Map<Gateway, Set<GatewayMapFlow>>
      incomings; // this keep tracks of the incoming flows of a gate
  private Map<Gateway, Set<GatewayMapFlow>>
      outgoings; // this keep tracks of the outgoing flows of a gate
  ...
  public boolean generateMap(BPMNDiagram diagram) {
    ...
    //        successively we will edit concurrently the gateway map and the BPMN diagram as a
    // mirror
    ...
    //        the input diagram is saved as object attribute and given in input to the diagram
    // handler
    bpmnDiagram = diagram;
    ...
    while (toVisit.size() != 0) {
      ...
      for (BPMNNode child : children.get(entry)) {
        ...
        tasks.add(0, entry);
        while (!gateways.contains(tmpChild) && (children.get(tmpChild).size() == 1)) {
          //                    tmpChild is not a gateway neither the main exit neither something
          // weird
          tasks.add(0, tmpChild);
          tmpChild = children.get(tmpChild).get(0);
        }
        tasks.add(0, tmpChild);
        //                tasks is an array list with a gateway in position 0, and a gateway in
        // position length-1
        //                in between it contains all the activities (if any) executed between the
        // two gateway
        //                note: last element is the entry gateway, the first element is the exit
        // gateway
        ...
        if (gateways.contains(tmpChild))
          this.addFlow(entry, tmpChild, tasks.get(tasks.size() - 2), tasks.get(1));
  ...
  private boolean init(BPMNDiagram diagram) {
    ...
    for (Flow f : diagram.getFlows()) {
      ...
      if (!children.containsKey(src)) children.put(src, new ArrayList<>());
      children.get(src).add(tgt);

      if (!parents.containsKey(tgt)) parents.put(tgt, new ArrayList<>());
  ...
  private void exploreMap() {
    ...
    //        finally we generate the dominators tree for we will need it later for the conversion
    // of the IORs
  ...
  private boolean exploreLoops(
      Gateway entry,
      HashSet<Gateway> unvisited,
      HashSet<Gateway> visiting,
      HashMap<Gateway, Boolean> visitedGates,
      HashSet<GatewayMapFlow> visitedEdges,
      HashSet<GatewayMapFlow> loopEdges,
    ...
    //        if we reached the exit gateway, it means we can definitely go forward to the end
    // event,
    ...
    for (GatewayMapFlow oflow : outgoings.get(entry)) {
      ...
      if (unvisited.contains(next)) {
        if (exploreLoops(
            next, unvisited, visiting, visitedGates, visitedEdges, loopEdges, forwardEdges)) {
          loopEdge = true;
          loopEdges.add(oflow);
        } else {
          forwardEdge = true;
    ...
    for (GatewayMapFlow iflow : incomings.get(entry))
      if (!visitedEdges.contains(iflow)) {
        visited = false;
        break;
      }
  ...
  private void normalizeLoopJoins() {
    ...
    for (Gateway join : new HashSet<>(gateways))
      if (incomings.get(join).size() > 1) {
        ...
        if (!loops.isEmpty()) {
          if (fwds.size() > 1) {
            //                        if the join has more than 1 forward edge, and at least 1 loop
            // edge
            //                        we split it into two new joins
            counter++;

            //                        we create a new gateway both in the BPMN diagram and the
            // gateway map
            loopJoin = bpmnDiagram.addGateway("lj+" + counter, Gateway.GatewayType.DATABASED);
            this.addGateway(loopJoin);

            //                        editing the gateway map
            //                        this is a 1-length loop because we removed the join/split
            // gateways
            ...
            for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> oe :
                new HashSet<>(bpmnDiagram.getOutEdges(join))) {
              //                            this is a 1-length loop because we removed the
              // join/split gateways
            ...
            //                        here we are redirecting the loop edges found before, detected
            // through the source node
            for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> ie :
                new HashSet<>(bpmnDiagram.getInEdges(join)))
            ...
            loopJoins.add(join);
            if (join.getGatewayType() == Gateway.GatewayType.INCLUSIVE)
  ...
  private void populatedIORHierarchy(Gateway entry) {
    ...
    while (!toVisit.isEmpty()) {
      ...
      for (Gateway next : successors.get(entry)) {
        ...
        else if ((gatesDepth.get(next) < sDepth) && !loopJoins.contains(next))
          gatesDepth.put(next, sDepth);
  ...
  private void generateDominatorsTree() {
    ...
    for (Gateway g : gateways) {
      GID++;
      reachableGates.put(GID, new ArrayList<>());
      gateIDs.put(g, GID);
      idToGate.put(GID, g);
    }
  ...
  public void detectAndReplaceIORs() {
    ...
    //        then, we follow each incoming flow up to the dominator of the IOR through a backward
    // exploration
    //        for each XOR gateway found during this backward exploration
    //        for each of its outgoing edges that do not lead to the incoming flow from which we
    // started the backward exploration
    //        we place a token generator on that outgoing edge
    //        algorithm in: "The Difficulty of Replacing an Inclusive OR-Join" (Favre et Volzer) -
    // p. 12(167)
    while (!iorsHierarchy.isEmpty()) {
      ...
      for (GatewayMapFlow igmf : new HashSet<>(incomings.get(ior))) {
        if (igmf.loop) {
          //                    Favre et Volzer do not handle loops, therefore we need a special
          // routine for them:
          //                    in presence of IORs loop-joins we do not look for a dominator
          //                    we turn them into XORs, and successively if they inject into
          // fragments with an IOR
          //                    join as fragment exit we put a token generator which will throw a
          // token to each
          //                    other incoming flow of the IOR fragment exit
          //                    memo: we should never get inside this IF, because we set all the
          // loop-joins to XORs before
        ...
        last = igmf.last;
        for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> oe :
            new HashSet<>(bpmnDiagram.getOutEdges(last))) {
          if (oe.getTarget() == ior) {
            //                        editing the BPMN diagram
  ...
  private Gateway.GatewayType replaceIOR(
      Gateway dominator,
      int iorDepth,
      Map<Gateway, Set<Gateway>> toVisit,
      Map<Gateway, Set<Gateway>> visitedGates,
    ...
    //        algorithm is in: "The Difficulty of Replacing an Inclusive OR-Join" (Favre et Volzer)
    // - p. 12(167)
        ...
        for (GatewayMapFlow igmf : incomings.get(g)) {
          ...
          if (!visitedFlows.get(xor).contains(igmf)) {
            ...
            if (loopJoins.contains(src) && !loopInjections.containsKey(src)) {
              loopInjections.put(src, new HashSet<>());
              for (GatewayMapFlow injection : incomings.get(src))
                if (!gatesDepth.containsKey(injection.src)
                    || (gatesDepth.get(injection.src) > iorDepth))
                  loopInjections.get(src).add(injection);
            }
            if (!visitedGates.get(xor).contains(src)
                && (!gatesDepth.containsKey(src) || !(gatesDepth.get(src) > iorDepth))) {
              //                        NOTE: if we are on a loop edge, and its depth is greater
              // than the IOR to replace
              //                              it means this is an injecting back-edge, therefore we
              // do not visit that gateway
              visitedGates.get(xor).add(src);
              if ((src.getGatewayType() == Gateway.GatewayType.PARALLEL)
                  && (outgoings.get(src).size() > 1)) ANDs.add(src);
              tmp.add(src);
              empty = false;
            }
          }
    ...
    if (empty) {
      ...
      if (checkXOR(visitedGates, visitedFlows, ANDs)) {
        ...
        //                THIS METHOD IS BUGGY, return INCLUSIVE to be sure not to break the final
        // model
      ...
      for (Gateway xor : visitedGates.keySet()) {
        ...
        for (Gateway g : visitedGates.get(xor)) {
          ...
          if ((g.getGatewayType() == Gateway.GatewayType.PARALLEL)
              || (outgoings.get(g).size() == 1)) continue;
          //                    if we are here, it means g is a decision (XOR split gateway)
          for (GatewayMapFlow of : outgoings.get(g)) {
            //                        debug("DEBUG - outgoing: " + of.getSource().getLabel() + " ->
            // " + of.getTarget().getLabel());
            if (visitedFlows.get(xor).contains(of)
                || ((g == dominator) && !domFrontier.contains(of))) continue;
      ...
      for (Gateway loopInj : loopInjections.keySet()) {
        ...
        if (!tmp.isEmpty())
          loopChanges.add(new MultipleTokenGen(tmp, loopInjections.get(loopInj), loopInj));
      }
  ...
  private boolean checkXOR(
    ...
    //        ANDs is the set of the AND-splits that we encountered between the IOR and its
    // dominator
    for (Gateway and : ANDs) {
      ...
      //            visitedGates is the set of all the gateways encountered between the IOR and its
      // dominator
      for (Gateway xor : visitedGates.keySet()) {
        //                System.out.println("DEBUG - XOR: " + xor.getLabel());
        //                V1 contains all the outgoing flows of an AND gateway that are incoming to
        // this XOR gateway
        V1 = new IntHashSet();
        //                U1 contains all the outgoing flows of an AND gateway that are incoming to
        // a different XOR gateway
        U1 = new IntHashSet();
        //                visited flows of a certain gateway are the incoming edges to that gateway,
        // because we are doing a backward exploration
        ...
        // visitedEdgesIDs.get(and).get(xor));
        //                System.out.println("DEBUG - Unvisited: " +
        // unvisitedEdgesIDs.get(and).get(xor));
      }
    }
  ...
  private void placeTokenGenerator(SingleTokenGen tg) {
    ...
    if ((eFlow.first instanceof Gateway)
        && (((Gateway) eFlow.first).getGatewayType() == Gateway.GatewayType.PARALLEL)
        && (outgoings.get(eFlow.first).size() > 1)) and = (Gateway) eFlow.first;
    else {
      ...
      for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> oe :
          new HashSet<>(bpmnDiagram.getOutEdges(eGate)))
  ...
  private void placeMultipleTokenGenerator(MultipleTokenGen mtg) {
    ...
    if (injections.size() > 1) {
      ...
      //            then we redirect all the loop incoming flows of the loop-join to the new XOR in
      // this gateway map
      ...
      //            then we redirect all the loop incoming flows of the loop-join to the new AND on
      // the BPMN diagram
      for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> ie :
          new HashSet<>(bpmnDiagram.getInEdges(loopInjection)))
        if (srcs.contains(ie.getSource())) {
          bpmnDiagram.removeEdge(ie);
    ...
    for (GatewayMapFlow inj : injections) {
      if ((inj.last instanceof Gateway)
          && (((Gateway) inj.last).getGatewayType() == Gateway.GatewayType.PARALLEL)
          && (outgoings.get(inj.last).size() > 1)) {
  ...
  private void addGateway(Gateway gate) {
    gateways.add(gate);
    incomings.put(gate, new HashSet<>());
    outgoings.put(gate, new HashSet<>());
    successors.put(gate, new HashSet<>());
    predecessors.put(gate, new HashSet<>());
    map.put(gate, new HashMap<>());
  }
  ...
  private GatewayMapFlow addFlow(BPMNNode src, BPMNNode tgt, BPMNNode first, BPMNNode last) {
    //        NOTE: this method does not work if the SRC or the TGT are not present in the gateway
    // map
    ...
    if (!map.get(entry).containsKey(exit)) map.get(entry).put(exit, new HashSet<>());
  ...
  public void checkANDLoops(boolean fix) {
      ...
      if (g.getGatewayType() == Gateway.GatewayType.PARALLEL && outgoings.get(g).size() > 1) {
        for (GatewayMapFlow gmf : outgoings.get(g))
          if (gmf.isLoop() && gmf.getTarget().getGatewayType() == Gateway.GatewayType.DATABASED) {
            System.out.println(
                "DEBUG - found an AND with a loop-back outgoing edge (TGT = "
                    + gmf.last.getLabel()
                    + ")");
    ...
    if (fix) {
      for (Gateway g : loopANDs.keySet()) {
        ...
        for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> ie :
            new HashSet<>(bpmnDiagram.getInEdges(g))) {
          bpmnDiagram.addFlow(ie.getSource(), xor, "");
          bpmnDiagram.removeEdge(ie);
        }
        ...
        for (GatewayMapFlow ogmf : loopANDs.get(g)) {
          first = ogmf.first;
          for (BPMNEdge<? extends BPMNNode, ? extends BPMNNode> oe :
              new HashSet<>(bpmnDiagram.getInEdges(first)))
            if (oe.getSource() == g) {
              bpmnDiagram.removeEdge(oe);
  ...
  private static class GatewayMapFlow implements Comparable {
    ...
    BPMNNode
        first; // this is the successor BPMN node seen from the entry of this Flow (that is src)
    BPMNNode
        last; // this is the predecessor BPMN node seen from the exit of this Flow (that is: tgt)
  ...
  private static class IORsHierarchy {
    ...
    void put(Gateway gate, int depth) {
      ...
      if (!hierarchy.containsKey(depth)) hierarchy.put(depth, new ArrayList<>());
